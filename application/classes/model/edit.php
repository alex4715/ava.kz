...
class Model_Edit extends Model_Image{
    
    private $_post;
    public $errors;
    
    public function dynImg($id = null){
        $file = $this->getFullPath('original');
        // $img = Image::factory($file, 'imagick');
         $img = new Imagick($file);
         ($id == 1)?$img->blurImage(5,3):$img;
         //($id == 0)?$this->save($img, 'end'):$img;
         return $img;
    }
    ...
    public function doSave(){
        ...
        $this->save(Cookie::get('i').'.jpg', 'original', $crop);
    }
   */
}